<template>
  <div
    class="forecast row justify-content-center align-self-center"
    :class="{ 'loading-color': loading }"
  >
    <div v-for="forecast in updatedForecasts" v-bind:key="forecast.id" class="forecast-details">
      ...
      <div class="degrees text-center">
        ...
        <div
          v-if="!loading"
        >{{changeScale ? forecast.highTempC : forecast.highTempF}}&deg; | {{changeScale ? forecast.lowTempC : forecast.lowTempF}}&deg;</div>
      </div>
