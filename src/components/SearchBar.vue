<template>
  <div class="wrap row justify-content-center align-self-center">
    <div class="search col-12 col-lg-5 col-md-8 col-sm-12">
      <input
        v-model="destination"
        type="text"
        class="search-term"
        placeholder="Location (City, Zip, Country...)"
    ...
    <p
      v-if="searchTimeStamp != ''"
      class="updated-on-text col-12 text-center"
    >as of {{searchTimeStamp}}</p>
  </div>
</template>
      ...
      $.ajax({
        ...
        success: function(resp) {
          ...
          if (Object.keys(resp).length !== 0) {
            ...
            self.searchTimeStamp = moment(resp.currently.time * 1000).format(
              "MM/DD/YY h:mm:ss A"
            );
        ...
        axios
          .get(
            "https://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=" +
              self.destination +
              "&key=AIzaSyB4v4851zzgqR4vYwG3FZbEckU2yMavgJY"
          )
          .then(resp => {
